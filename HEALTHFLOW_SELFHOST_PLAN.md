...
1. **Staging gate + manual promote — DEFERRED (user Jul 4).** Auto-deploy-to-prod is fine at 3 clients (small blast radius); revisit when crossing **~5–8 clients**, where a bad deploy hurts enough tenants to justify the friction. Then: GHA builds → deploys staging → manual promote to prod.
2. **Observability floor — ADOPT (user Jul 4).** Enable `pg_stat_statements` (one line; currently off → zero slow-query visibility), add a searchable log drain, and one external uptime alert (UptimeRobot). Lightweight.
3. **Pin Node in the self-host Dockerfile — ADOPT as `node:20-slim`, NOT 22 (user Jul 4, codebase-verified).** Current `Dockerfile` pins **`node:18-slim`** (both stages) → source of the engine warnings; deps (pino 10, AWS SDK v3) already need ≥20. **Node 20 clears every warning with ZERO dep bumps:** its Debian base ships **OpenSSL 3.0**, which *also* resolves the Prisma "openssl-1.1.x" warning; **Prisma 5.7 fully supports Node 20** (Node 22 predates Prisma 5.7's official support → would force a Prisma upgrade, more risk); `chromium` package present on the Node-20 base so PDF unaffected. → change both `FROM node:18-slim` to `node:20-slim`.
4. **API abuse hardening — MOSTLY ALREADY BUILT (codebase-verified Jul 4).** `src/middleware/rateLimit.ts` already exists: Redis-backed + in-memory fallback, fails safe, wired onto the internet-facing token routes (report/bill/statement/QR-gateway downloads) **and** login (`loginIpRateLimit` 10/min/IP). The only *additive* piece is **fail2ban** — an OS/firewall-level ban for IPs that hammer the host (e.g. the `/wp-admin` bots), sitting below the app-layer limiter. *(Rec: add fail2ban at migration; optionally extend app limits to the whole authed `/api/*` surface — low priority.)*
...
## Public token-gateway security (audited Jul 4)

The `/r/:token` QR gateway plus the three token download routes (`/reports/:token`, `/bills/view/:token`, `/statements/view/:token`) are the largest **unauthenticated PII surface** in the product (patient reports, itemized bills, whole-period doctor statements). Three code-grounded audits (entropy, rate-limit/enumeration, lifecycle/scope) plus a red-team pass were run against the live backend. Every finding below is verified against actual `file:line`, not catalog copy.

### 1. Current state (what's already solid — do not rebuild)

The foundation is genuinely strong; five headline worries are **disproven or closed**, do not spend effort re-litigating them:

- **The "un-throttled poller" worry is FALSE.** Both the landing `GET /r/:token` (`reportGateway.ts:256-259`) and the polling `GET /r/:token/status` (`reportGateway.ts:217-221`) carry `gatewayIpRateLimit` (30/60s per IP) **and** `gatewayTokenRateLimit` (15/60s per IP+token). Config at `reportGateway.ts:60-70,72-82`. All four download routes carry equivalent throttles (`reportDownload.ts:133,173,218`; `billDownload.ts:59-62`; `statementDownload.ts:238-241`). The limiters are present on every route including the poller — they are keyed *wrong* (§2), but they exist.
- **Token forgery/prediction is CLOSED.** All three active services mint via `crypto.randomBytes(32)` (256-bit CSPRNG), not cuid/uuid/sequential (`billAccessService.ts:20-26`, identical in report/statement), with a P2002 collision-retry loop (≤10x). No structure, sequence, or embedded PII to predict.
- **Hashed at rest.** Only the SHA-256 hash is stored; the raw token lives only in the URL (`billAccessService.ts:28-29`; `schema.prisma:1202,1680,1699`). A DB read alone yields no usable link (except legacy plaintext report rows — §2 [LOW]).
- **Cross-PATIENT reach is CLOSED single-tenant.** Tokens are type-scoped by table/service (bill→`visitId`, report→`reportVersionId`, statement→`payoutId`); no user-supplied record id appears in any route; each token resolves to exactly one record. A bill token physically cannot reach the report table.
- **No PII in the token.** Random bytes, not a JWT carrying `patientId`. The old JWT flow (`tokenService.ts`) is DEAD and wired to nothing.
- **Live per-test cancel-filtering CLOSED at the gateway.** Cancelled/films-only orders are dropped at every render (`reportGateway.ts:306-319`); a snapshot requires `status===FINALIZED`. A stale token never surfaces a since-cancelled test. (This is TEST-level only; document-level void/refund/supersede is NOT covered — §2 [HIGH].)
- **Report-side append-only audit.** `ReportAccessLog` with type/via/IP/UA/userId (`schema.prisma:1218-1233`).
- **`Cache-Control: no-store`** on every token response; rate-limit store **fails closed** in production (`rateLimit.ts:59-60,77,117-122`, Redis required + 503 when `NODE_ENV=production` — the throttle cannot be disabled by knocking out Redis).

### 2. Real gaps (code-verified only)

**Matters TODAY (single-tenant Sobhana):**

- **[HIGH] Non-expiring, non-revocable links replay forever — the live exploit.** Every mint passes `expiresAt = NULL` (`reportGateway.ts:328,343`; `reportQrService.ts:90`; `notificationService.ts:160,414,527,632`) and no model has `revokedAt` (`schema.prisma:1200-1214,1678-1692,1697-1711`). `validateBillToken` only checks expiry (`billAccessService.ts:85-90`). The per-test cancel filter does NOT cover document-level events: a voided/refunded bill PDF stays publicly live via `/bills/view/:token`, a superseded report keeps prior tokens valid (the gateway mints a fresh non-expiring one every hit), and a departed doctor's whole-period statement link never dies. One forwarded WhatsApp link = a permanent, unlimited-pull firehose.
- **[HIGH] Per-IP limiter is spoofable → enumeration/scale-exfil throttle is effectively absent.** `app.set('trust proxy', true)` (`index.ts:91`) trusts all hops, so `req.ip` is the leftmost client-supplied `X-Forwarded-For`. `getClientIp()` returns `req.ip` (`rateLimit.ts:84-86`) and every IP limiter keys on it — a fresh XFF per request lands each hit in a new 30/min bucket across all four routes. The token-keyed limiter keys on (IP, token) (`reportGateway.ts:76`, `rateLimit.ts:166`), so a new token per guess is always a new bucket — it only caps repeated hits on an *already-known* token and gives **zero** anti-guessing value. *Honest caveat: with 71.5-bit tokens (below), blind brute-force is still computationally infeasible online even with the throttle gone — so this is defense-in-depth loss, not a standalone break, until a partial-token leak or oracle-narrowing turns it live.*
- **[MEDIUM] Silent existence/state oracle with no trace.** `/r/:token/status` returns `{state:'invalid'}` vs `waiting|partial|ready|none` and writes **no access log** (`reportGateway.ts:217-253`, no `recordBillAccess`) — mass probing validates a token + reveals report lifecycle without a download and leaves zero forensic trace. `reportDownload.ts:66-84` additionally leaks a distinct 403 "not finalized" state; every route is a 404-vs-200/302 existence oracle.
- **[MEDIUM] Sub-floor token entropy ~71.5 bits.** `generateToken()` strips base64url to alphanumeric and truncates to 12 chars: 62^12 ≈ 71.5 bits (`billAccessService.ts:20-26`), below the 128-bit floor for unauthenticated bearer tokens. Still CSPRNG-derived and unforgeable, but it is the ONLY surviving layer once the throttle bypass is used.
- **[MEDIUM] Token leakage via Referer.** Branded pages load `fonts.googleapis.com` cross-origin (`reportGateway.ts:127-129`) with the token in the URL path and no `Referrer-Policy` on any token route → the full `/r/:token` URL can leak in the Referer to a third party.
- **[MEDIUM] Bill/statement audit is weak and mutable.** Only overwritten columns `accessCount`/`lastAccessedAt`/`lastAccessedIp`, no UA, no per-event history (`billAccessService.ts:102-109`; `statementAccessService.ts:98-105`). Scaled scraping of a known link cannot be reconstructed after the fact (reports do have append-only `ReportAccessLog`).
- **[LOW] Legacy plaintext report tokens partially defeat hash-at-rest.** `reportAccessService.findTokenRecord()` falls back to `WHERE token = rawToken` (`reportAccessService.ts:52-74`); migration to hash is passive (only on next visit), so un-revisited rows stay cleartext indefinitely — a DB read yields directly usable links for those rows. Bill/statement services already dropped this fallback.
- **[LOW] BillAccessToken carries dual scope.** One bill token gates both the bill PDF and the `/r/:token` report gateway (`reportQrService.ts:90` reuses `existingToken`; `reportGateway.ts:266` is the sole auth). Forwarding a bill link also unlocks report PDFs — blast radius exceeds the token's label.

**Only matters at MULTI-TENANT (HealthFlow SaaS):**

- **[HIGH] Token models have NO tenant binding.** `ReportAccessToken`/`BillAccessToken`/`StatementAccessToken` carry only token + expiresAt, no `tenantId`/`branchId` (`schema.prisma:1200-1214,1678-1692,1697-1711`), unlike every other branch-scoped model. Every resolve is a **global** `findUnique` on the token hash (`billAccessService.ts:38-41` and peers) and these public routes carry no JWT/tenant context. On the planned single `api.healthflow.in` with any shared-DB tenancy, a tenant-A token validates globally → cross-tenant PII read. Not exploitable today (single tenant); it is the one PII surface with no tenant guard.

### 3. Hardening to add (decisive, ordered; effort S/M/L)

Every still-exploitable finding maps to exactly one item below.

1. **Default TTL at mint** — closes **[HIGH] replay-forever** (lifecycle half). Set a default inside each `create*` service when the caller omits: reports/bills 90–180d, statements 30–60d; backfill `expiresAt` on existing rows. The `@@index([expiresAt])` + validate checks already enforce it. **M.**
2. **Revocation** — closes **[HIGH] replay after void/refund/supersede** (lifecycle half). Add `revokedAt` to all three models, check in every `validate*()`, and wire revoke to bill void/refund, visit cancel, report supersede; expire prior report tokens when a newer FINALIZED version exists. **M.**
3. **`trust proxy = 1`** (`index.ts:91`) — closes **[HIGH] IP-spoof throttle bypass** (control half). One line restores the real edge IP as `req.ip` across every IP limiter and all four routes. **S.**
4. **Failure-keyed enumeration cap** in `rateLimit.ts` — closes **[HIGH] IP-spoof throttle bypass** (guessing half). A per-IP counter that increments on 404/validation-failure, low threshold (~20/min), block/tarpit on trip; wire into all four routes' landing + status handlers. This is the actual anti-guessing control — the token-keyed limiter cannot count guesses. **M.**
5. **Raise entropy floor** — closes **[MEDIUM] sub-floor entropy**. Remove `.replace()` + `.substring(0,12)` in all three `generateToken()`s → full base64url (~256 bits) or ≥22 chars (≥131 bits). New tokens only; old ones keep working. **S.**
6. **Kill the oracles** — closes **[MEDIUM] silent existence/state oracle**. Collapse `reportDownload`'s 403 "not finalized" into the generic 404 path (`reportDownload.ts:66-84`), emit a sampled access/anomaly log on `/status`, and return a uniform response for invalid-vs-not-ready. **S–M.**
7. **`Referrer-Policy: no-referrer` + self-host the font** on all four routes — closes **[MEDIUM] Referer leak** (`reportGateway.ts:127-129`). **S.**
8. **Append-only bill/statement audit** — closes **[MEDIUM] weak mutable audit**. Shared `TokenAccessLog` (IP + UA + timestamp), matching reports' `ReportAccessLog`. **M.**
9. **Backfill-hash legacy report tokens, then delete the plaintext fallback** (`reportAccessService.ts:52-74`) — closes **[LOW] plaintext-at-rest**, matches bill/statement. **S.**
10. **Distinct scopes for bill-PDF vs report-gateway** — closes **[LOW] dual-scope BillAccessToken**. Separate tokens (or a scope flag checked at `reportGateway.ts:266`) so a forwarded bill link does not unlock report PDFs. **S.**
11. **Tenant-binding (SaaS gate)** — closes **[HIGH] cross-tenant resolution**. Add `tenantId` to each token model and resolve within tenant (derive tenant from host/subdomain on these public routes), OR hard-enforce **DB-per-tenant** so these routes physically hit one tenant's DB — which the self-host plan already favors, making this mostly a verification task if honored. Decide **before** tenant #2 onboards. **L** (shared-DB) / **S** (if DB-per-tenant is enforced).

### 4. Verdict

**Not safe to expose broadly as-is — but the foundation is strong and the wall is short.** Closed and confirmed: tokens are unforgeable/unpredictable (CSPRNG + hash-at-rest + collision retry), type-scoped so there is no cross-patient reach single-tenant, stale tokens live-filter per-test cancellations, the throttle fails closed in prod, and `no-store` is universal. The "un-throttled poller" and "weak token source" worries are **disproven**.

Skeptical correction to the earlier framing: even with the throttle fully bypassed, blind brute-force of the ~71.5-bit space is computationally infeasible online, so **pure token-guessing is NOT the live exploit**. The concretely exploitable vectors today are (1) permanent, non-expiring, non-revocable links that replay forever and survive bill void/refund and report supersede, and (2) a silent, un-logged status/existence oracle that validates and narrows tokens without a trace. At multi-tenant a third appears: (3) global token resolution with no `tenantId` binding = cross-tenant PII unless DB-per-tenant is hard-enforced. The spoofable throttle + non-counting limiter are a real defense-in-depth collapse that must be fixed, but they are the layer *behind* the sub-floor entropy, not a standalone break.

**Minimum bar before onboarding tenant #2:** ship items 1–5 and 11 — default TTL, revocation wired to void/refund/cancel/supersede, `trust proxy=1`, failure-keyed enumeration cap, entropy back to ≥128 bits, and tenant isolation. Items 6–10 (oracle-collapse, Referrer-Policy, append-only audit, plaintext backfill, scope split) can follow the first tenant but should land before scale. If DB-per-tenant (per the self-host plan) is genuinely enforced, item 11 collapses from a schema migration to a verification task — bank that by keeping the DB-per-tenant decision firm.

### 5. Decisions (user, Jul 4)

1. **No TTL — keep `expiresAt = NULL`.** Patients access reports on their own schedule; links do not auto-expire. Item 1 (default TTL) is **dropped**. Instead: **add `revokedAt` to all three token models and wire revocation to bill CANCELLATION and REFUND** (item 2, scoped) — those events kill the link; everything else stays live indefinitely. Per-test cancel is already filtered at render; this adds the document-level (void/refund) kill. (Report-supersede can reuse the same `revokedAt` later if wanted; not in scope now.)
2. **Fix the spoofable throttle (items 3+4).** `trust proxy = 1` (one line, restores the real client IP as `req.ip`) + the failure-keyed enumeration cap in `rateLimit.ts`.
3. **Raise entropy + kill the oracle (items 5+6).** Drop the `.replace()/.substring(0,12)` in the three `generateToken()`s → full base64url (≥128 bits, new tokens only); collapse `/r/:token/status` invalid-vs-not-ready into one uniform response + a sampled access/anomaly log.
4. **Tenant isolation via DB-per-tenant (item 11).** Confirmed — no per-token `tenantId` migration; DB-per-tenant physically scopes token resolution to one tenant's DB. Verification task, not a schema change.

Deferred (post first tenant, per verdict): item 7 Referrer-Policy, item 8 append-only bill/statement audit, item 9 plaintext-token backfill, item 10 bill/report scope split.
