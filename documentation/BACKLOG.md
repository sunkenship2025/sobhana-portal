...
- **Status badge system** — only `PayoutsTable` pills tokenized. Broader: `StatusBadge` is underused across pages, the pending hue is inverted, and `.status-unpaid` is missing.
...
- **Accessibility pass** — icon-only buttons missing `aria-label`, ~136 inputs without associated `<Label htmlFor>`, contrast on branch accents + status badges, sidebar `focus-visible` ring, skip-to-content link.
