...
- **Loading/Empty states** — `LoadingState`/`EmptyState` adopted only on owner config pages; other pages still use ad-hoc `Loader2`/bare text. Migrate incrementally.
- **Status badge system** — `PayoutsTable` pills tokenized to success/warning. **Needs a design decision, not a sweep:** the remaining hardcoded green/amber pills are mostly *categorical* (workflow modes, product types) and should stay; the "inverted pending hue" is a choice — `--status-pending` is blue while the tokenized PayoutsTable now uses amber/`warning`, so they mismatch. Pick the canonical pending color (blue vs amber), then align `StatusBadge` + the pills. `.status-unpaid` token exists but its class is unused (no real gap).
