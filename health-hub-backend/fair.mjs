...
for (const [name, cases] of [['held-out (32)', CEIL], ['owner phrasing (20)', PAIRS]]) {
  let v1 = 0, v2 = 0, ms1 = 0, ms2 = 0; const m1 = [], m2 = [];
  const tasks = cases.map((c) => async () => {
    ...
    let t = Date.now();
    try { const a = await sqlAnswer(k, c.q); ms1 += Date.now()-t;
      if (a.kind === 'sql' && score(want, nums(a.rows))) v1++; else m1.push(c.id); } catch { m1.push(c.id); }
    t = Date.now();
    try { const b = await analyse(c.q, {}); ms2 += Date.now()-t;
  ...
  console.log(`${name}`);
  console.log(`  V1 single-call : ${v1}/${cases.length}  ${(ms1/cases.length/1000).toFixed(1)}s`);
  console.log(`  V2 analyst     : ${v2}/${cases.length}  ${(ms2/cases.length/1000).toFixed(1)}s`);
  console.log(`  V1 only misses : ${m1.filter(x=>!m2.includes(x)).sort().join(' ') || '—'}`);
  console.log(`  V2 only misses : ${m2.filter(x=>!m1.includes(x)).sort().join(' ') || '—'}`);
}
process.exit(0);
