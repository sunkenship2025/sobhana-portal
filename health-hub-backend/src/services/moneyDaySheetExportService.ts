...
export async function buildDaySheetWorkbook(data: DaySheetResponse): Promise<Buffer> {
  ...
  ws.columns = [
    ...
    { header: 'Due from', key: 'dueFrom', width: 14 },
    ...
    { header: 'Reversed (₹)', key: 'reversed', width: 13, style: { numFmt: RUPEE_FMT } },
    { header: 'Net (₹)', key: 'net', width: 13, style: { numFmt: RUPEE_FMT } },
  ...
  data.rows.forEach((r, i) => {
    ws.addRow({
      ...
      dueFrom: r.dueFrom ? istDate(r.dueFrom) : '',
      patient: patientDisplay(r),
      referredBy: r.referredBy ?? '—',
      branch: r.branchCode,
      tests: r.tests,
      gross: r.dueFrom ? null : Math.round(r.grossInPaise) / 100,
      discount: r.dueFrom ? null : Math.round(r.discountInPaise) / 100,
      reversed: r.dueFrom ? null : Math.round(r.reversedInPaise) / 100,
      net: r.dueFrom ? null : Math.round(r.netInPaise) / 100,
  ...
  const totals = ws.addRow({
    ...
    reversed: Math.round(data.totals.reversedInPaise) / 100,
    net: Math.round(data.totals.netInPaise) / 100,
