...
// ============================================================================
// PORTAL ACCESS — invite, then credentials
// ============================================================================

/**
 * Invite a newly added team member. Carries NO credentials.
 *
 * Credentials cannot travel in a template at all: Meta rejects a utility template
 * that contains them (INCORRECT_CATEGORY), and the authentication category has a
 * fixed body and a 15-character parameter cap that a login email cannot fit. The
 * only way to send them on WhatsApp is a free-form message, and that is allowed
 * solely inside the 24-hour window a person opens by replying. So this asks for
 * the reply; sendPortalCredentials answers it.
 */
export async function sendPortalInvite(input: {
  ...
  try {
    ...
    const formattedPhone = formatPhoneForWhatsApp(input.phone);
    if (!formattedPhone) return { success: false, error: 'That phone number is not usable on WhatsApp' };

    await createAndSendTemplateMessage({
      patientId: null,
      phone: formattedPhone,
      templateName: 'portal_access_invite',
      templateParams: { name: input.name, userId: input.userId },
      contextId: input.userId,
      contextType: MessageContextType.REMINDER,
    ...
    log.warn({ err, userId: input.userId }, 'portal invite failed');
    return { success: false, error: reason };
  }
}

/**
 * Send the sign-in details as a FREE-FORM message. Only valid because the person
 * just replied, which opened the 24-hour service window.
 *
 * The password is generated by the caller at this moment and hashed onto the user
 * in the same step, so it is never at rest in plaintext anywhere — not in
 * MessageLog, not in the audit log, and not while the invite sits pending.
 */
export async function sendPortalCredentials(
  ...
  const text =
    `Your Sobhana Diagnostics portal sign-in details:\n\n` +
    `Email: ${email}\n` +
    `Password: ${password}\n\n` +
