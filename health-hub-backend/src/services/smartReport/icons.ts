 ...
 * left of centre here.
 */
// Kept >=20px apart so the 15px numbered dots never overlap — they did at first,
// which read as a smudge exactly where the figure is supposed to be precise.
...
const RULES: [RegExp, string, string, Anchor][] = [
  ...
  [/liver|lft|hepatic/i, 'i-liver', '#B5651D', { x: 70, y: 92 }],
  [/kidney|renal|kft|rft/i, 'i-kidney', '#8E44AD', { x: 112, y: 112 }],
  [/thyroid|tsh|\bt3\b|\bt4\b/i, 'i-thyroid', '#1E88E5', { x: 90, y: 48 }],
  [/vitamin\s*d|calcium|bone/i, 'i-vitd', '#F9A825', { x: 66, y: 140 }],
  ...
  [/lung|pulmon|pft|spiro/i, 'i-thyroid', '#26A69A', { x: 68, y: 72 }],
];
