...
const near = (hay, w) => hay.some((g) => Math.abs(g - w) <= Math.max(0.02, Math.abs(w) * 0.005) || Math.abs(g - w/100) <= Math.max(0.02, Math.abs(w/100) * 0.005) || Math.abs(g*100 - w) <= Math.max(0.02, Math.abs(w) * 0.005));
for (const [name, cases] of [['held-out (32)', CEIL], ['owner phrasing (20)', PAIRS]]) {
  let ok = 0, cal = 0, ms = 0; const miss = [];
  const tasks = cases.map((c) => async () => {
    ...
    try { const a = await analyse(c.q, {});
      ...
      // score the EVIDENCE the UI renders (raw rows included), not just the narration
      const got = nums({ t: a.text, s: a.evidence?.map((e) => e.summary), d: a.evidence?.map((e) => e.data) });
      const hits = want.filter((w) => near(got, w)).length;
      if (want.length === 0 || hits === want.length || (want.length > 2 && hits / want.length >= 0.8)) ok++; else miss.push(c.id);
    } catch { miss.push(c.id + ':err'); }
  });
  let n = 0; await Promise.all(Array.from({length:2}, async () => { while (n < tasks.length) await tasks[n++](); }));
  console.log(`${name}: ${ok}/${cases.length} · ${(cal/cases.length).toFixed(1)} calls · ${(ms/cases.length/1000).toFixed(1)}s`);
